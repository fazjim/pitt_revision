...
public class CRFTagger2 {
	 ...
	 */
	public CRFTagger2(String modelFile){
...
public List<Double> tagProb(String fileName) throws IOException {
		List<Double> taggedProbs = 
		...
		for (ItemSequence xseq: taggingSequences.getFirst()) {
			tagger.set(xseq);
			StringList labels = tagger.viterbi();
			double prob = tagger.probability(labels);
			taggedProbs.add(prob);
		}
		return taggedProbs;
	}
}
