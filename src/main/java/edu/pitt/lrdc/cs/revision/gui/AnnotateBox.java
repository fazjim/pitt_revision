...
public class AnnotateBox extends JPanel {
	...
	public void loadTable() {
		for (int i = RevisionPurpose.START; i <= RevisionPurpose.END; i++) {
			...
			table.put(purposeName,
					new EditUnitV2(purposeName, ColorConstants.getColor(i)));
		}
	...
	public AnnotateBox() {
		...
		surfaceEUGroup.setBorder(BorderFactory
				.createTitledBorder("Surface Revisions"));
		bonusGroup = new JPanel();
		bonusGroup.setBorder(BorderFactory.createTitledBorder("Surface Bonus"));
		contentEUGroup = new JPanel();
		contentEUGroup.setBorder(BorderFactory
				.createTitledBorder("Content Revisions"));
	...
	public void reload(ArrayList<RevisionUnit> rus) {
		...
		for (int i = 0; i < rus.size(); i++) {
			...
			String name = RevisionPurpose.getPurposeName(ru
					.getRevision_purpose());
